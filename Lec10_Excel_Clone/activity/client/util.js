...
function updateChildrens(cellObject) {
  ...
  for (let i = 0; i < childrens.length; i++) {
    ...
    // ui set
    let childDiv = document.querySelector(
      `div[rid="${rowId}"][cid="${colId}"]`
    );
...
function setUI(){
    for(let i=0 ; i<100 ; i++){
        for(let j=0 ; j<26 ; j++){
            ...
            document.querySelector(`div[rid="${i}"][cid="${j}"]`).textContent = cellObject.value;
        }
    }
}
...
function initUI() {
  ...
  for (let i = 0; i < 100; i++) {
    ui += `<div class="row">`;
    for (let j = 0; j < 26; j++) {
        ui += `<div class="cell" contentEditable="true" rid=${i} cid=${j}> </div>`
    }
    ui += `</div>`;
  }
  cells.innerHTML = ui;
